...
(10843,32-33): run-time error JS1013: Syntax error in regular expression: ;
 */
/*!
...
var virtualKeyboardDetector = function (inputStart, inputEnd, blacklist) {
    if (blacklist === void 0) { blacklist = ['Shift']; }
    var func = function (ele, callback) {
        var timeout = null;
        var buffer = [];
        var handler = function (event) {
            var target = $(event.target);
            if (buffer.length || event.key == inputStart) {
                //if (target.is('input') || target.parent('input').length > 0) {
                event.preventDefault();
                //}
                event.stopPropagation();
                if (event.key == inputStart) {
                    buffer = [];
                }
                // cancel the current timeout
                timeout && clearTimeout(timeout);
                // register the timeout to cancel if we don't type another key
                timeout = setTimeout(function () {
                    buffer = [];
                }, 300);
                if (blacklist.indexOf(event.key) == -1) {
                    buffer.push(event.key);
                    var inputEndLength = buffer.length - inputEnd.length;
                    if (buffer.slice(inputEndLength).equals(inputEnd)) {
                        callback(buffer.slice(1, inputEndLength).join(''));
                        buffer = [];
                    }
                }
            }
        };
        ele.addEventListener('keydown', handler);
        return {
            destroy: function () {
                ele.removeEventListener('keydown', handler);
            }
        };
    };
    return func;
};
var wristbandDetector = virtualKeyboardDetector(';', ['?']);
//# sourceMappingURL=wristband.js.map;
var linq = {
...
Date.prototype.add = function (years, months, days, hours, minutes, seconds) {
    return new Date(this.year + years, this.month + months, this.date + days, this.hours + hours, this.minutes + minutes, this.seconds + seconds);
};
Date.prototype.format = function (format, i18nSettings) {
    return fecha.format(this, format, i18nSettings);
};
['Years', 'Months', 'Days', 'Hours', 'Minutes', 'Seconds'].forEach(function (it, idx) {
    Date.prototype['add' + it] = function (val) {
        var args = [0, 0, 0, 0, 0];
        args.splice(idx, 0, val);
        return Date.prototype.add.apply(this, args);
    };
});
//# sourceMappingURL=date-manipulation.js.map;
